...
export function Dropzone({ onFilesSelected, onFilesRejected, disabled }: DropzoneProps) {
  ...
  const { getRootProps, getInputProps, isDragActive } =
    useDropzone({
      ...
      onDropRejected,
      maxSize: MAX_SIZE,
      // Datasets only — the backend enforces the same set, this just keeps
      // the file picker focused on queryable formats.
      accept: {
        "text/csv": [".csv"],
          ...
          <>
            ...
            <div>
              <p className="text-base font-semibold">
                Drag &amp; drop datasets here, or click to browse
              </p>
              <p className="text-xs text-muted-foreground mt-1 font-mono">
                CSV · JSON · log · Parquet · max 100 MB
              </p>
            </div>
          </>
        )}

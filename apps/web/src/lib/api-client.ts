...
export async function getHealth() {
  return apiFetch<{ status: string; b2_connected: boolean; bucket: string }>(
    "/health",
  );
}
...
export async function runQuery(sql: string, maxRows?: number) {
  return apiFetch<QueryResult>("/query/run", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ sql, max_rows: maxRows ?? null }),
  });
}

export async function materializeQuery(sql: string, name: string) {
  return apiFetch<SavedQuery>("/query/materialize", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
...
export async function getQueryHistory(limit = 50) {
  return apiFetch<SavedQuery[]>(`/query/history?limit=${limit}`);
}
